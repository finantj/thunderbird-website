...
function ensureBadgeRecord(all, badgeId) {
  ...
  if (!all.badges[badgeId]) {
    all.badges[badgeId] = { modules: {}, total: 0, percent: 0, purchased: false, assistant: {} };
  }
...
export function getBadgeProgress(badgeId) {
  const all = loadAll();
  const badge = (all.badges && all.badges[badgeId]) || null;
  if (!badge) {
    return { purchased: false, modules: {}, percent: 0, total: 0 };
  }
  return badge;
}
...
export function updateModule(badgeId, moduleId, payload = {}) {
  // convenience wrapper that marks a module done and saves other payload fields
...
/*
  Assistant thread helpers stored alongside local progress.
  getAssistantThread(badgeId, moduleId) -> { moduleId, messages: [] }
  appendAssistantMessages(badgeId, moduleId, messages) -> updated thread
*/
export function getAssistantThread(badgeId, moduleId) {
  if (!badgeId) return { moduleId: moduleId || "general", messages: [] };
  const all = loadAll();
  const badge = (all.badges && all.badges[badgeId]) || {};
  const assistant = badge.assistant || {};
  const key = moduleId || "general";
  const thread = assistant[key] || { messages: [] };
...
export function appendAssistantMessages(badgeId, moduleId, messages = []) {
  if (!badgeId || !Array.isArray(messages) || !messages.length) {
  ...
  const key = moduleId || "general";
  const thread = badge.assistant[key] || { messages: [] };
  thread.messages = Array.isArray(thread.messages) ? thread.messages : [];
  const sanitized = messages.map((m) => {
    // minimal sanitization: keep role/content/createdAt/suggestions/resources
    const entry = {
      role: m.role === "assistant" ? "assistant" : "scout",
      content: typeof m.content === "string" ? m.content : "",
    };
    if (m.createdAt) entry.createdAt = m.createdAt;
    if (Array.isArray(m.suggestions) && m.suggestions.length) entry.suggestions = m.suggestions.slice(0, 8);
    if (Array.isArray(m.resources) && m.resources.length) entry.resources = m.resources.slice(0, 8);
    return entry;
  }).filter(Boolean);
  ...
  badge.updatedAt = thread.updatedAt;
  all.badges[badgeId] = badge;
  saveAll(all);

  // also queue a server-side assistant event for centrally storing transcripts
  const scoutId = all.scout ? ensureScoutId(all.scout) : null;
  if (scoutId) {
    queueProgressSync({
      type: "assistant",
